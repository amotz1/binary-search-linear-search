...
# it loop through the list checking every element if it is the element that i specified and if it is it returns the index of that number in the list.
...
def test_linear_search():
    ...
    for number_to_search in list_of_numbers_to_search:
        assert (linear_search(testcase, number_to_search) == testcase.index(number_to_search))
    assert (linear_search(testcase,3) == False)
...
# this is the public variable. it gets a sorted list it then use the __binary_search function with it and the arguments 0 to the end of the sorted list -1.
def binary_search(sorted_list, number_to_search):
    return __binary_search(sorted_list, 0, len(sorted_list) - 1, number_to_search)



# using a variable testcase that contains a list i created another variable list_of_numberstosearch that is a list of numberstosearch.
...
def test_binary_search():
    ...
    list_of_numberstosearch = [0, 1, 8, 5, 4, 8, 6, 3, 1]
    for number_to_search in list_of_numberstosearch:
        assert (binary_search(testcase, number_to_search))
    assert binary_search(testcase,10) == False
